...
import AgentProfileSkeleton from "../components/loading/AgentProfileSkeleton";
...
export default function AgentProfile() {
  ...
  useEffect(() => {
    const getAgent = async () => {
      setLoading(true);
      try {
        const result = await fetch(`${API_BASE_URL}/api/get-user/${id}`);
        ...
        if (data && data.user._id) {
          const res = await fetch(
            `${API_BASE_URL}/api/user-properties/${data.user._id}`,
          );
          if (res.ok) {
  ...
  return (
    <main className="bg-[#FBFCFC] min-h-screen pt-6 md:pt-12 pb-16 md:pb-24">
      <ContentWrapper>
        <div className="flex flex-wrap items-center gap-2 text-[9px] md:text-[10px] font-black uppercase tracking-[0.15em] md:tracking-[0.2em] text-gray-400 mb-8 md:mb-12">
          <a href="/agents" className="hover:text-[#327878] transition-colors">
            Agents
          </a>
          <span className="w-1 h-1 rounded-full bg-gray-300" />
          <span className="text-[#327878]">
            {agent.name} {agent.surname}
          </span>
        </div>

        <div className="flex flex-col lg:flex-row gap-8 lg:gap-16 items-start mb-16 lg:mb-24">
          <div className="w-full lg:w-[400px] shrink-0 lg:sticky lg:top-32">
            <div className="bg-white rounded-4xl md:rounded-[2.5rem] border border-gray-100 p-5 md:p-8 shadow-[0_20px_50px_rgba(0,0,0,0.03)]">
              ...
              <div className="space-y-4 pt-8 border-t border-gray-50">
                <a
                  ...
                  className="flex items-center gap-4 p-4 rounded-2xl bg-[#f9fbfb] hover:bg-[#327878] hover:text-white transition-all group"
                >
                  <FontAwesomeIcon
                    icon={faEnvelope}
                    className="text-[#327878] group-hover:text-white"
                  />
                  ...
                  (social, i) =>
                    social.url && (
                      <a
                        ...
                        target="_blank"
                        rel="noreferrer"
                        className="w-10 h-10 rounded-xl border border-gray-100 flex items-center justify-center text-gray-400 hover:text-[#327878] hover:border-[#327878] transition-all"
                      >
          ...
          <div className="flex-1 space-y-16">
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4">
              {[
                ...
                <div
                  key={i}
                  className="bg-white p-4 md:p-6 rounded-2xl md:rounded-3xl border border-gray-50 shadow-sm"
                >
                  <div className="text-[#327878] mb-3 opacity-30">
                    <FontAwesomeIcon icon={stat.icon} />
                  </div>
                  <p className="text-xl md:text-2xl font-black text-slate-900 leading-none mb-2">
                    {stat.value}
                  </p>
            ...
            <div className="space-y-10">
              <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-3">
                ...
                <p className="text-xs font-bold text-[#327878] uppercase tracking-widest">
                  {properties.length} Results Found
                </p>
              </div>

              {properties.length > 0 ? (
                <RevealGroup className="grid grid-cols-1 md:grid-cols-2 gap-5 md:gap-8">
                  {properties.map((el) => (
